...
// Tenta pegar a URL do env, ou do app.json extra, ou usa fallback
const API_BASE_URL = 
  process.env.EXPO_PUBLIC_API_URL || 
  Constants.expoConfig?.extra?.apiUrl ||
  'https://api.breakerbot.online';
...
async function fetchApi<T>(endpoint: string, options?: RequestInit): Promise<T> {
  ...
  try {
    ...
    console.log('[API] Status:', response.status, '| Resposta:', text.substring(0, 200));
    ...
    try {
      ...
      console.error('[API] Erro ao parsear JSON. Resposta recebida:', text.substring(0, 500));
      throw new Error(`API retornou resposta inválida (status ${response.status}). Verifique se a URL está correta: ${API_BASE_URL}`);
    }
